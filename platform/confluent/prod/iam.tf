...
resource "confluent_role_binding" "admin-sa-clusteradmin-rb" {
  ...
  crn_pattern = confluent_kafka_cluster.prod.rbac_crn
}
...
resource "confluent_api_key" "admin-sa-api-key" {
  ...
  managed_resource {
    ...
    api_version = confluent_kafka_cluster.prod.api_version
    kind        = confluent_kafka_cluster.prod.kind
...
resource "confluent_role_binding" "consumer-developer-read-from-topic-rb" {
  ...
  crn_pattern = "${confluent_kafka_cluster.prod.rbac_crn}/kafka=${confluent_kafka_cluster.prod.id}/topic=${confluent_kafka_topic.topic.topic_name}"
}
...
resource "confluent_role_binding" "consumer-developer-read-from-group-rb" {
  principal = "User:${confluent_service_account.consumer-sa.id}"
  role_name = "DeveloperRead"
  crn_pattern = "${confluent_kafka_cluster.prod.rbac_crn}/kafka=${confluent_kafka_cluster.prod.id}/group=confluent_cli_consumer_*"
}
